...
async def _update_research_run(run_id: str, status: str, result: Optional[dict[str, Any]] = None, error: Optional[str] = None, data_hash: Optional[str] = None, user_id: str = "default") -> None:
    import asyncpg
    ...
    try:
        await conn.execute(
            "UPDATE quant_research_runs SET status = $2, result = COALESCE($3::jsonb, result), error = $4, data_hash = COALESCE($5, data_hash), completed_at = CASE WHEN $2 IN ('completed', 'failed') THEN NOW() ELSE completed_at END WHERE id = $1 AND user_id = $6",
            run_id, status, json.dumps(result, default=str) if result is not None else None, error, data_hash, user_id,
        )
    finally:
        await conn.close()


def update_research_run(run_id: str, status: str, result: Optional[dict[str, Any]] = None, error: Optional[str] = None, data_hash: Optional[str] = None, user_id: str = "default") -> None:
    import asyncio
