...
export default function ValidationPage() {
  ...
  return <main className="min-h-screen bg-[#f5f5f2] text-stone-900"><div className="min-h-screen flex">
    <aside className="hidden lg:flex w-60 shrink-0 border-r border-stone-200 bg-white p-5 flex-col"><Link href="/" className="text-lg font-bold">Quant<span className="text-orange-600">Alpha</span></Link><p className="text-[10px] uppercase tracking-[0.18em] text-stone-400 mt-1">Research workspace</p><nav className="mt-8 space-y-1" aria-label="Primary navigation">{[["/", "Overview"], ["/signals", "Signals"], ["/research", "Research"], ["/validation", "Validation"], ["/reports", "Reports"]].map(([href, label]) => <Link key={href} href={href} className={`block rounded-lg px-3 py-2.5 text-sm font-semibold ${href === "/validation" ? "bg-orange-50 text-orange-700" : "text-stone-600 hover:bg-stone-50"}`}>{label}</Link>)}</nav><div className="mt-auto rounded-xl bg-stone-50 p-3 text-xs text-stone-500">Define a rule, test it on verified data, then review the evidence.</div></aside>
