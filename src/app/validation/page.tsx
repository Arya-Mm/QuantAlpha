        ...
        <div className="max-w-6xl mx-auto flex flex-col gap-6">
          <header>
            <div className="flex justify-between gap-4">
              <div>
                <p className="text-xs uppercase tracking-[0.18em] text-orange-700 font-bold">Research workbench</p>
                <h1 className="text-3xl font-bold mt-2">Signal validation</h1>
                <p className="text-stone-600 mt-2 max-w-2xl">Choose an interpretable strategy, run it on real market data, and inspect CPCV, PBO, and DSR evidence.</p>
              </div>
              <Link href="/reports" className="h-fit rounded-lg border border-stone-300 bg-white px-4 py-2 text-sm font-semibold">View reports</Link>
            </div>
          </header>

          <form onSubmit={saveStrategy} className="bg-white border border-stone-200 rounded-xl p-6 grid gap-4 md:grid-cols-4 shadow-sm">
            <div className="md:col-span-4">
              <h2 className="font-bold">1. Choose a strategy</h2>
              <p className="text-sm text-stone-500 mt-1">Templates make the research hypothesis explicit; you can inspect the generated formula before saving.</p>
            </div>
            <label className="md:col-span-4 text-sm font-semibold">
              Strategy template
              <select aria-label="Strategy template" value={strategy.id} onChange={(event) => { const preset = presets.find((item) => item.id === event.target.value); if (preset) setStrategy(preset); }} className="mt-2 w-full border border-stone-300 rounded-lg p-2 font-normal">
            ...
            <input aria-label="Strategy name" placeholder="Strategy name" value={strategy.name} onChange={(event) => setStrategy({ ...strategy, name: event.target.value })} className="border border-stone-300 rounded-lg p-2" required />
            <input aria-label="Strategy formula" placeholder="Formula" value={strategy.formula} onChange={(event) => setStrategy({ ...strategy, formula: event.target.value })} className="border border-stone-300 rounded-lg p-2 md:col-span-2" required />
            <button type="submit" className="rounded-lg border border-orange-600 text-orange-700 px-4 py-2 font-semibold hover:bg-orange-50">Save strategy</button>
          </form>

          <form onSubmit={runValidation} className="bg-white border border-stone-200 rounded-xl p-6 grid gap-5 md:grid-cols-4 shadow-sm">
            <div className="md:col-span-4">
              <h2 className="font-bold">2. Run empirical validation</h2>
              <p className="text-sm text-stone-500 mt-1">The saved strategy is converted into next-bar positions, charged turnover costs, then tested statistically.</p>
            </div>
            <label className="flex flex-col gap-2 text-sm font-semibold">
              Strategy
              <select value={signalId} onChange={(event) => setSignalId(event.target.value)} className="border border-stone-300 rounded-lg p-2 font-normal" disabled={!signals.length}>
                <option value="">No strategies available</option>
                {signals.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
              </select>
            </label>
            <label className="flex flex-col gap-2 text-sm font-semibold">
              Dataset
              <select value={datasetId} onChange={(event) => setDatasetId(event.target.value)} className="border border-stone-300 rounded-lg p-2 font-normal" disabled={!datasets.length}>
                <option value="">No datasets available</option>
                {datasets.map((item) => <option key={item.id} value={item.id}>{item.label} · {item.source}</option>)}
              </select>
            </label>
            <label className="flex flex-col gap-2 text-sm font-semibold">
              Start date
              <input type="date" value={dates.start} onChange={(event) => setDates({ ...dates, start: event.target.value })} className="border border-stone-300 rounded-lg p-2 font-normal" />
            </label>
            <label className="flex flex-col gap-2 text-sm font-semibold">
              End date
              <input type="date" value={dates.end} onChange={(event) => setDates({ ...dates, end: event.target.value })} className="border border-stone-300 rounded-lg p-2 font-normal" />
            </label>
            <button type="submit" className="md:col-span-4 rounded-lg bg-orange-600 text-white px-4 py-3 font-semibold disabled:opacity-50 hover:bg-orange-700 cursor-pointer" disabled={busy || !signals.length || !datasets.length}>
              {busy ? "Starting validation..." : "Run real validation"}
            </button>
          </form>
          ...
          {selectedRun && (
            <section className="bg-white border border-stone-200 rounded-xl p-5">
              <div className="flex justify-between">
                <div>
                  <h2 className="font-bold">Selected run</h2>
                  <p className="text-xs text-stone-500 font-mono mt-1">{selectedRun.id}</p>
                </div>
                <span className="rounded-full px-3 py-1 text-xs font-bold bg-amber-100 text-amber-800">{selectedRun.status}</span>
              </div>
              {selectedRun.status !== "completed" && selectedRun.status !== "failed" && (
                <div className="mt-4">
                  <div className="flex justify-between text-sm">
                    <span>{selectedRun.result?.progress || 5}%</span>
                    <span>{selectedRun.result?.stage || "Waiting for worker"}</span>
                  </div>
                  <div className="h-2 bg-stone-100 rounded-full mt-2">
                    <div className="h-full bg-orange-500" style={{ width: `${selectedRun.result?.progress || 5}%` }} />
                  </div>
                </div>
              )}
              {selectedRun.status === "completed" && (
                <div className="mt-5 grid gap-3 md:grid-cols-4">
                  {[["Sharpe", details?.sharpe_ratio], ["DSR", details?.dsr], ["PBO", details?.pbo], ["CPCV paths", details?.n_cpcv_paths]].map(([label, value]) => (
                    <div key={label} className="rounded-lg bg-stone-50 p-4">
                      <p className="text-xs uppercase text-stone-500">{label}</p>
                      <p className="text-2xl font-bold mt-1">{value ?? "Unavailable"}</p>
                    </div>
                  ))}
                </div>
              )}
          ...
          <section className="bg-white border border-stone-200 rounded-xl overflow-hidden shadow-sm">
            <div className="px-5 py-4 border-b border-stone-200 flex justify-between">
              <div>
                ...
                <p className="text-sm text-stone-500 mt-1">Saved runs remain tied to their strategy and data provenance.</p>
              </div>
              <button type="button" onClick={() => void load()} className="text-sm text-orange-700 font-semibold cursor-pointer">Refresh</button>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  {runs.map((run) => (
                    <tr key={run.id} onClick={() => setSelectedRunId(run.id)} className="border-t border-stone-100 cursor-pointer hover:bg-orange-50">
              ...
              {!runs.length && <p className="p-8 text-center text-sm text-stone-500">No persisted runs yet.</p>}
            </div>
          </section>
        </div>
      </div>
